...
def Get_fake_func(fakeType: str, fake: fk):
    return getattr(fake, fakeType)
...
def GetRandomValueFaker(fakeType: str, fake: fk, listOfParameters: list):
    ...
    try:
        randomvalue = None
        fakeFunc = Get_fake_func(fake, fakeType)
        ...
        msn.print_message(
            f'Error in the function getRandomValueFaker.',
            f'Exception: {e}'
        )
...
def GetAmountOfRegisters(JSON_ALL_TABLES: dict, tableName: str) -> int:
    ...
    try:
        ...
        msn.print_message(
            ...
            f'The default value was set to {amountOfRegisters}',
            f'Exception: {e}'
        )
    finally:
...
def GetRandomPkOrFk(listOfPk: dict, sourceTable: str, sourceField: str) -> str:
    ...
    try:
        ...
        msn.print_message(
            f'Error getting the list of pk from the table {sourceTable}.', 
...
def GetColumnsNames(JSON_ALL_TABLES: dict, tableName:str) -> str:
    ...
    try:
        ...
        msn.print_message(f'Error in the function GetColumnsNames. Error: {e}')
    finally:
        return columnsNames
...
def CreateRecordString(actualRecord:list, pksOfThisTable:list, values:list, jsonVariable:dict, jsonOfThisTable:dict, tableName:str, element:str, fake:faker) -> list:
    ...
    if('ispk' in element.keys()):
        ...
        uniqueID = fake.unique.Get_fake_func(element["faketype"], fake)(listOfParameters)
